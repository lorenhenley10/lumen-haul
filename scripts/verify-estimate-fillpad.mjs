#!/usr/bin/env node
//
// Check every published estimate in public/q/ still has a working fill-in pad.
//
// A estimate in public/q/ is a standing link that outlives the working file it
// was cut from — documents/estimates/ is gitignored and will not exist in a
...
// are the same ones publish-estimate.mjs refuses to write without, imported
// rather than copied.
//
// Usage:
//   node scripts/verify-estimate-fillpad.mjs                 # everything in public/q
//   node scripts/verify-estimate-fillpad.mjs path/to/one.html
//

import { readFileSync, readdirSync } from "node:fs";
import { join } from "node:path";

import { fillPadChecks } from "./estimate-checks.mjs";

const ESTIMATE_DIR = "public/q";

const listPublished = () => {
  try {
    return readdirSync(ESTIMATE_DIR)
      .filter((name) => name.endsWith(".html"))
      .map((name) => join(ESTIMATE_DIR, name));
  } catch {
...
if (!targets.length) {
  console.log(`  no estimates in ${ESTIMATE_DIR}/ — nothing to check`);
  process.exit(0);
}
